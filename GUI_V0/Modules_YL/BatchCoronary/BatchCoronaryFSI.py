...
class BatchCoronary:
    def __init__(self, UI=None, Hedys=None):
        ...
        self.ui.ChooseBloodPressureAndHeartRatePathBtn_BPP.clicked.connect(lambda: self.ChooseBloodPressureAndHeartRatePath())
        ...
        self.ui.ChooseWallMaskPathBtn_BPP.clicked.connect(lambda: self.ChooseWallMaskPath())
        self.ui.ChooseSolidsetPathBtn_BPP.clicked.connect(lambda: self.ChooseSolidsetPath())
    ...
    def initProcess(self):
        ...
        self.WallMaskTable = []
        self.WallMaskTablePath = None
        self.WallMaskProcessPath = None
        self.WallStackTissueTable = []
        self.WallStackTissueTablePath = None
        self.WallSTLTable = []
        self.WallSTLTablePath = None
        self.LumenCutTable = []
        self.LumenCutTablePath = None
        self.WallSmoothTable = []
        self.WallSmoothTablePath = None
        self.WallStackLumenTable = []
        self.WallStackLumenTablePath = None
        self.WallCutTable = []
        self.WallCutTablePath = None
        self.WallFillGapTable = []
        self.WallFillGapTablePath = None
        self.WallMeshTable = []
        self.WallMeshTablePath = None
        self.WallFindFaceTable = []
        self.WallFindFaceTablePath = None
        self.WallsolidsetPath = None
        self.WallCooTable = []
        self.WallCooTablePath = None
        self.WallElemIDTable = []
        self.WallElemIDTablePath = None
        self.WallGenerateTable = []
        self.WallGenerateTablePath = None
        self.NodeMatchingTable = []
        self.NodeMatchingTablePath = None
        self.WallDataOutputTable = []
        self.WallDataOutputTablePath = None
        self.WallVTUTable = []
        self.WallVTUTablePath = None
    ...
    # used to choose Wall Mask path
    def ChooseWallMaskPath(self):
        filename = Save_Load_File.OpenFilePathQt(dispMsg="Choose csv file",
                                                 fileTypes="All files (*.*);; CSV files(*.csv)",
                                                 fileObj=self.ui, qtObj=True)
        self.ui.WallMaskPath_Txt_BPP.setPlainText(filename)

    # used to choose Solid set path
    def ChooseSolidsetPath(self):
        filename = Save_Load_File.OpenFilePathQt(dispMsg="Choose csv file",
                                                 fileTypes="All files (*.*);; CSV files(*.csv)",
                                                 fileObj=self.ui, qtObj=True)
        self.ui.SolidsetPath_Txt_BPP.setPlainText(filename)
    ...
    def CreateFolders(self, CasePath=None):
        ...
        solidPath = os.path.join(CasePath, 'solid')
        solidLumenCutPath = os.path.join(solidPath, 'LumenCut')
        solidWallADINAPath = os.path.join(solidPath, 'WallADINA')
        solidWallCooPath = os.path.join(solidPath, 'WallCoo')
        solidWallCutPath = os.path.join(solidPath, 'WallCut')
        solidWallDataOutputPath = os.path.join(solidPath, 'WallDataOutput')
        solidWallElemIDPath = os.path.join(solidPath, 'WallElemID')
        solidWallFillGapPath = os.path.join(solidPath, 'WallFillGap')
        solidWallFindFacePath = os.path.join(solidPath, 'WallFindFace')
        solidWallMaskPath = os.path.join(solidPath, 'WallMask')
        solidWallMeshPath = os.path.join(solidPath, 'WallMesh')
        solidWallNodeMatchingPath = os.path.join(solidPath, 'WallNodeMatching')
        solidWallresultPath = os.path.join(solidPath, 'WallResult')
        solidWallSmoothPath = os.path.join(solidPath, 'WallSmooth')
        solidWallStackLumenPath = os.path.join(solidPath, 'WallStackLumen')
        solidWallSTLPath = os.path.join(solidPath, 'WallSTL')
        solidWallVtuPath = os.path.join(solidPath, 'WallVtu')
        solidWallWithTissuePath = os.path.join(solidPath, 'WallWithTissue')
        ...
        self.CreateSingleFolder(solidPath)
        self.CreateSingleFolder(solidLumenCutPath)
        self.CreateSingleFolder(solidWallADINAPath)
        self.CreateSingleFolder(solidWallCooPath)
        self.CreateSingleFolder(solidWallCutPath)
        self.CreateSingleFolder(solidWallDataOutputPath)
        self.CreateSingleFolder(solidWallElemIDPath)
        self.CreateSingleFolder(solidWallFillGapPath)
        self.CreateSingleFolder(solidWallFindFacePath)
        self.CreateSingleFolder(solidWallMaskPath)
        self.CreateSingleFolder(solidWallMeshPath)
        self.CreateSingleFolder(solidWallNodeMatchingPath)
        self.CreateSingleFolder(solidWallresultPath)
        self.CreateSingleFolder(solidWallSmoothPath)
        self.CreateSingleFolder(solidWallStackLumenPath)
        self.CreateSingleFolder(solidWallSTLPath)
        self.CreateSingleFolder(solidWallVtuPath)
        self.CreateSingleFolder(solidWallWithTissuePath)
    ...
    def StoreListInformation(self):
        ...
        self.WallMaskTablePath = os.path.join(self.TableDir, 'solid_WallMask.csv')
        self.WallMaskProcessPath = os.path.join(self.TableDir, 'LumenDilation.csv')
        self.WallStackTissueTablePath = os.path.join(self.TableDir, 'solid_WallStackTissue.csv')
        self.WallSTLTablePath = os.path.join(self.TableDir, 'solid_WallSTL.csv')
        self.LumenCutTablePath = os.path.join(self.TableDir, 'solid_LumenCut.csv')
        self.WallSmoothTablePath = os.path.join(self.TableDir, 'solid_WallSmooth.csv')
        self.WallStackLumenTablePath = os.path.join(self.TableDir, 'solid_WallStackLumen.csv')
        self.WallCutTablePath = os.path.join(self.TableDir, 'solid_WallCut.csv')
        self.WallFillGapTablePath = os.path.join(self.TableDir, 'solid_WallFillGap.csv')
        self.WallMeshTablePath = os.path.join(self.TableDir, 'solid_WallMesh.csv')
        self.WallFindFaceTablePath = os.path.join(self.TableDir, 'solid_WallFindFace.csv')
        self.WallsolidsetPath = os.path.join(self.TableDir, 'setsolid.csv')
        self.WallCooTablePath = os.path.join(self.TableDir, 'solid_WallCoo.csv')
        self.WallElemIDTablePath = os.path.join(self.TableDir, 'solid_WallElemID.csv')
        self.WallGenerateTablePath = os.path.join(self.TableDir, 'solid_WallGenerate.csv')
        self.NodeMatchingTablePath = os.path.join(self.TableDir, 'solid_NodeMatching.csv')
        self.WallDataOutputTablePath = os.path.join(self.TableDir, 'solid_WallDataOutput.csv')
        self.WallVTUTablePath = os.path.join(self.TableDir, 'solid_WallVTU.csv')
    ...
    def GenerateTable(self):
        ...
        for CSVPath in self.CSVPathList:
            ...
            tmpIFRPostNpyColumn = [os.path.join(DCMDirPath, 'IFRPostProgress'), True,
                                   os.path.join(DCMDirPath, 'FFRPostProgress/Npy'), TimeStart_IFR, TimeStop_IFR]
            ...
            ndarrstr = 'ndCooOSI_2dndarr.npy,ndCooRRT_2dndarr.npy,ndCooTAWSS_2dndarr.npy,' \
                       'ndCooTAWSS_VectorMag_2dndarr.npy,ndTimeAvePressureCoo_2dndarr.npy,' \
                       'ndTimeDecile1PressureCoo_2dndarr.npy,ndTimeDecile1WSSCoo_2dndarr.npy,' \
                       'ndTimeDecile2PressureCoo_2dndarr.npy,ndTimeDecile2WSSCoo_2dndarr.npy,' \
                       'ndTimeDecile3PressureCoo_2dndarr.npy,ndTimeDecile3WSSCoo_2dndarr.npy,' \
                       'ndTimeDecile4PressureCoo_2dndarr.npy,ndTimeDecile4WSSCoo_2dndarr.npy,' \
                       'ndTimeDecile6PressureCoo_2dndarr.npy,ndTimeDecile6WSSCoo_2dndarr.npy,' \
                       'ndTimeDecile7PressureCoo_2dndarr.npy,ndTimeDecile7WSSCoo_2dndarr.npy,' \
                       'ndTimeDecile8PressureCoo_2dndarr.npy,ndTimeDecile8WSSCoo_2dndarr.npy,' \
                       'ndTimeDecile9PressureCoo_2dndarr.npy,ndTimeDecile9WSSCoo_2dndarr.npy,' \
                       'ndTimeEnergyPressureCoo_2dndarr.npy,ndTimeEnergyWSSCoo_2dndarr.npy,' \
                       'ndTimeEntropyPressureCoo_2dndarr.npy,ndTimeEntropyWSSCoo_2dndarr.npy,' \
                       'ndTimeGMeanPressureCoo_2dndarr.npy,ndTimeGMeanWSSCoo_2dndarr.npy,' \
                       'ndTimeHMeanPressureCoo_2dndarr.npy,ndTimeHMeanWSSCoo_2dndarr.npy,' \
                       'ndTimeIQRPressureCoo_2dndarr.npy,ndTimeIQRWSSCoo_2dndarr.npy,' \
                       'ndTimeKurtosisPressureCoo_2dndarr.npy,ndTimeKurtosisWSSCoo_2dndarr.npy,' \
                       'ndTimeMaxPressureCoo_2dndarr.npy,ndTimeMaxWSSCoo_2dndarr.npy,' \
                       'ndTimeMedianPressureCoo_2dndarr.npy,ndTimeMedianWSSCoo_2dndarr.npy,' \
                       'ndTimeMinPressureCoo_2dndarr.npy,ndTimeMinWSSCoo_2dndarr.npy,' \
                       'ndTimeModePressureCoo_2dndarr.npy,ndTimeModeWSSCoo_2dndarr.npy,' \
                       'ndTimePTPPressureCoo_2dndarr.npy,ndTimePTPWSSCoo_2dndarr.npy,' \
                       'ndTimeQ1PressureCoo_2dndarr.npy,ndTimeQ1WSSCoo_2dndarr.npy,ndTimeQ3PressureCoo_2dndarr.npy,' \
                       'ndTimeQ3WSSCoo_2dndarr.npy,ndTimeRMSPressureCoo_2dndarr.npy,ndTimeRMSWSSCoo_2dndarr.npy,' \
                       'ndTimeSEPressureCoo_2dndarr.npy,ndTimeSEWSSCoo_2dndarr.npy,ndTimeSkewPressureCoo_2dndarr.npy,' \
                       'ndTimeSkewWSSCoo_2dndarr.npy,ndTimeSTDPressureCoo_2dndarr.npy,ndTimeSTDWSSCoo_2dndarr.npy,' \
                       'ndTimeTriMeanPressureCoo_2dndarr.npy,ndTimeTriMeanWSSCoo_2dndarr.npy,' \
                       'ndTimeVarPressureCoo_2dndarr.npy,ndTimeVarWSSCoo_2dndarr.npy'
            niistr = 'CTOSI.nii.gz,CTRRT.nii.gz,CTTAWSS.nii.gz,CTTAWSS_VectorMag.nii.gz,CTTimeAvePressure.nii.gz,' \
                     ...
                     'CTTimeMinWSS.nii.gz,CTTimeModePressure.nii.gz,CTTimeModeWSS.nii.gz,CTTimePTPPressure.nii.gz,' \
                     'CTTimePTPWSS.nii.gz,CTTimeQ1Pressure.nii.gz,CTTimeQ1WSS.nii.gz,CTTimeQ3Pressure.nii.gz,' \
                     'CTTimeQ3WSS.nii.gz,CTTimeRMSPressure.nii.gz,CTTimeRMSWSS.nii.gz,CTTimeSEPressure.nii.gz,' \
                     'CTTimeSEWSS.nii.gz,CTTimeSkewPressure.nii.gz,CTTimeSkewWSS.nii.gz,CTTimeSTDPressure.nii.gz,' \
            ...
            tmpMap_ResultColumn = [os.path.join(self.OriVolumePathList[loopIndex], 'OriVolumeCAVal.nii.gz'), False, 100,
                                   os.path.join(DCMDirPath, 'PostProgress/PostNpy'), ndarrstr, 666, 5, True, 1,
                                   os.path.join(DCMDirPath, 'PostProgress/MapCT'), False, niistr]
            tmpFFRMap_ResultColumn = [os.path.join(self.OriVolumePathList[loopIndex], 'OriVolumeCAVal.nii.gz'), False,
                                      100, os.path.join(DCMDirPath, 'FFRPostProgress/PostNpy'), ndarrstr, 666, 5, True,
                                      1, os.path.join(DCMDirPath, 'FFRPostProgress/MapCT'), False, niistr]
            tmpIFRMap_ResultColumn = [os.path.join(self.OriVolumePathList[loopIndex], 'OriVolumeCAVal.nii.gz'), False,
                                      100, os.path.join(DCMDirPath, 'IFRPostProgress/PostNpy'), ndarrstr, 666, 5, True,
                                      1, os.path.join(DCMDirPath, 'IFRPostProgress/MapCT'), False, niistr]
    ...
    def GenerateSolidTable(self):
        WallMask_Head = ['Image Path', 'Mask Path', 'Process Path', 'Fill Original Value', 'Output Directory',
                        'Output Name']
        WallStackTissue_Head = ['RefImgPath', 'LoadFolder', 'RefSegmentationName', 'ThresholdValues', 'NewValues',
                                'OutputFolder', 'OutRefName']
        WallSTL_Head = ['EXEPath', 'InputFilePath', 'OutputFilePath']
        LumenCut_Head = ['Mesh', 'InputStl(Mesh)', 'MeshExe', 'OutputFolder(Mesh)', 'Carotid(Mesh)', 'MeshSmooth(Mesh)',
                         'SurfaceClipper(Mesh)', 'CubeClipper(Mesh)', 'ClipperName(Mesh)', 'FlowExtension(Mesh)',
                         'SurfaceRemesh(Mesh)', 'TetralMeshGeneration(Mesh)', 'ConvertMeshType(Mesh)']
        WallSmooth_Head = ['Mesh', 'InputStl(Mesh)', 'MeshExe', 'OutputFolder(Mesh)', 'Carotid(Mesh)',
                           'MeshSmooth(Mesh)', 'IterationsSmooth(Mesh)', 'Relaxation(Mesh)', 'Boundarysmoothing(Mesh)',
                           'Normalize(Mesh)', 'SmoothName(Mesh)', 'SurfaceClipper(Mesh)', 'FlowExtension(Mesh)',
                           'SurfaceRemesh(Mesh)', 'TetralMeshGeneration(Mesh)', 'ConvertMeshType(Mesh)']
        WallStackLumen_Head = ['InPath', 'OutPath']
        WallCut_Head = ['MeshCut', 'InputStl(MeshCut)', 'OutputFolder(MeshCut)']
        WallFillGap_Head = ['MeshFillGap', 'InputStl(MeshFillGap)', 'OutputFolder(MeshFillGap)',
                            'Connectivity(MeshFillGap)', 'ConnectivityFloder(MeshFillGap)']
        WallMesh_Head = ['Mesh', 'InputStl(Mesh)', 'MeshExe', 'OutputFolder(Mesh)', 'Carotid(Mesh)', 'MeshSmooth(Mesh)',
                         'SurfaceClipper(Mesh)', 'FlowExtension(Mesh)', 'SurfaceRemesh(Mesh)',
                         'TetralMeshGeneration(Mesh)', 'RadiousAdaptive(Mesh)', 'EdgelengthMesh(Mesh)',
                         'EdgelengthFactorMesh(Mesh)', 'MinedgelengthMesh(Mesh)', 'MaxedgelengthMesh(Mesh)',
                         'BoundarylayerBool(Mesh)', 'Sublayers(Mesh)', 'Sublayerratio(Mesh)', 'skipremeshing(Mesh)',
                         'skipcapping(Mesh)', 'remeshcapsonly(Mesh)', 'FirstLayerThickness(Mesh)',
                         'BoundarylayeroncapsBool(Mesh)', 'MeshName(Mesh)', 'ConvertMeshType(Mesh)',
                         'ConvertMeshName(Mesh)']
        WallFindFace_Head = ['FindFace', 'InputNas(FindFace)', 'OutputFolder(FindFace)', 'Set(FindFace)',
                             'Tolerance(FindFace)']
        WallCoo_Head = ['filePaths', 'dirPaths']
        WallElemID_Head = ['TissueElemAssign', 'CooFolder(TissueElemAssign)', 'NpyFolder(TissueElemAssign)',
                           'OutputFolder(TissueElemAssign)']
        WallGenerate_Head = ['GenerateInput', 'CompIfoPath(GenerateInput)', 'InputNas(GenerateInput)',
                             'OutputFolder(GenerateInput)', 'TissueElemIdPath(GenerateInput)', 'AUIPath']
        NodeMatching_Head = ['NodeMatching', 'solidin(NodeMatching)', 'SolidIdb(NodeMatching)',
                             'SolidCompIfo(NodeMatching)', 'Offesetvalue(NodeMatching)', 'GroupNum(NodeMatching)',
                             'fluidlumenset(NodeMatching)', 'PressureID(NodeMatching)',
                             'EleFaceSetPrefix(NodeMatching)', 'TimeFunctionPrefix(NodeMatching)',
                             'LoadPrefix(NodeMatching)', 'AUIPath', 'fluidIdb(NodeMatching)', 'fluidPor(NodeMatching)',
                             'OutputFolder(NodeMatching)']
        WallDataOutput_Head = ['OutputFolder(DataOutput)', 'DataOutput', 'Fluid(DataOutput)', 'Idb Path(DataOutput)',
                               'Por Path(DataOutput)', 'AUI Path(DataOutput)', 'TimeStart(DataOutput)',
                               'TimeStop(DataOutput)']
        WallVTU_Head = ['OutputFolder', 'Savevtu', 'nasPath(Savevtu)', 'ParamsPath(Savevtu)', 'savePVDPath(Savevtu)',
                        'pvdName(Savevtu)', 'Fluidpvd(Savevtu)', 'Solidpvd(Savevtu)', 'getSigmaP1(Savevtu)',
                        'getEffectivestress(Savevtu)', 'getSolidmaxshearstress(Savevtu)', 'getSigmaNorm2(Savevtu)',
                        'PVDbinary(Savevtu)', 'SavePVD(Savevtu)', 'SaveFFRvtu(Savevtu)', 'Savevtufile(Savevtu)']

        MeshExe = self.ui.MeshExePath_Txt_BPP.toPlainText()
        AUIExe = self.ui.AUIPath_Txt_BPP.toPlainText()
        STLExe = self.ui.STLGExePath_Txt_BPP.toPlainText()

        for CSVPath in self.CSVPathList:
            loopIndex = self.CSVPathList.index(CSVPath)
            DCMDirPath = self.DCMDirList[loopIndex]

            tmpWallMaskColumn = [os.path.join(DCMDirPath, 'Preproc/OriVolume/OriVolumeCAVal.nii.gz'),
                                 os.path.join(DCMDirPath, 'Preproc/OriVolume/OriVolumeCAVal.nii.gz'),
                                 self.WallMaskProcessPath, 'Yes', os.path.join(DCMDirPath, 'solid/WallMask'), 'Wall']
            self.WallMaskTable.append(tmpWallMaskColumn)

            tmpWallStackTissueColumn = [os.path.join(DCMDirPath, 'Preproc/OriVolume/CTA.nii.gz'),
                                        os.path.join(DCMDirPath, 'solid/WallMask'),'WallVal.nii.gz',0,2,
                                        os.path.join(DCMDirPath, 'solid/WallWithTissue'),'WallWithTissue']
            self.WallStackTissueTable.append(tmpWallStackTissueColumn)

            tmpWallSTLColumn = [STLExe,os.path.join(DCMDirPath, 'solid/WallWithTissue/WallWithTissueVal.nii.gz'),
                                os.path.join(DCMDirPath, 'solid/WallSTL/Wall.stl')]
            self.WallSTLTable.append(tmpWallSTLColumn)

            tmpLumenCutColumn = [True, os.path.join(DCMDirPath, 'CFD/FindFace/Lumen.stl'), MeshExe,
                                 os.path.join(DCMDirPath, 'solid/LumenCut'), True, False, True, True, 'LumenCut',
                                 False, False, False, False]
            self.LumenCutTable.append(tmpLumenCutColumn)

            tmpWallSmoothColumn = [True, os.path.join(DCMDirPath, 'solid/WallSTL/Wall.stl'), MeshExe,
                                   os.path.join(DCMDirPath, 'solid/WallSmooth'), True, True, 15, 0.5, 0, 1,
                                   'WallSmooth', False, False, False, False, False]
            self.WallSmoothTable.append(tmpWallSmoothColumn)

            tempstllist = [os.path.join(DCMDirPath, 'solid/WallSmooth/WallSmooth.stl'),
                           os.path.join(DCMDirPath, 'solid/LumenCut/LumenCut.stl')]
            tmpWallStackLumenColumn = [','.join(tempstllist),
                                       os.path.join(DCMDirPath, 'solid/WallStackLumen/WallStack.stl')]
            self.WallStackLumenTable.append(tmpWallStackLumenColumn)

            tmpWallCutColumn = [True, os.path.join(DCMDirPath, 'solid/WallStackLumen/WallStack.stl'),
                                os.path.join(DCMDirPath, 'solid/WallCut')]
            self.WallCutTable.append(tmpWallCutColumn)

            tmpWallFillGapColumn = [True, os.path.join(DCMDirPath, 'solid/WallCut/lumenCut.stl'),
                                    os.path.join(DCMDirPath, 'solid/WallFillGap/FillGap.stl'), True,
                                    os.path.join(DCMDirPath, 'solid/WallFillGap/FillGapConnectivity.stl')]
            self.WallFillGapTable.append(tmpWallFillGapColumn)

            tmpWallMeshColumn = [True, os.path.join(DCMDirPath, 'solid/WallFillGap/FillGapConnectivity.stl'),
                                 MeshExe, os.path.join(DCMDirPath, 'solid/WallMesh'), True, False, False, False,
                                 False, True, False, 1, 0.3, 0.3, 4, 0, 3, 1.2, 1, 1, 1, 0.1, 0, 'solid.vtu', True,
                                 'solid.nas']
            self.WallMeshTable.append(tmpWallMeshColumn)

            tmpWallFindFaceColumn = [True, os.path.join(DCMDirPath, 'solid/WallMesh/solid.nas'),
                                     os.path.join(DCMDirPath, 'solid/WallFindFace'), self.WallsolidsetPath, 45]
            self.WallFindFaceTable.append(tmpWallFindFaceColumn)

            tmpWallCooColumn = [os.path.join(DCMDirPath, 'solid/WallWithTissue/WallWithTissueVal.nii.gz'),
                                os.path.join(DCMDirPath, 'solid/WallCoo')]
            self.WallCooTable.append(tmpWallCooColumn)

            tmpWallElemIDColumn = [True, os.path.join(DCMDirPath, 'solid/WallFindFace/3Dmesh.nas'),
                                   os.path.join(DCMDirPath, 'solid/WallCoo'),
                                   os.path.join(DCMDirPath, 'solid/WallElemID')]
            self.WallElemIDTable.append(tmpWallElemIDColumn)

            tmpWallGenerateColumn = [True, os.path.join(DCMDirPath, 'solid/WallFindFace/CompIfo.csv'),
                                     os.path.join(DCMDirPath, 'solid/WallFindFace/CFD.nas'),
                                     os.path.join(DCMDirPath, 'solid/WallADINA'),
                                     os.path.join(DCMDirPath, 'solid/WallElemID'),AUIExe]
            self.WallGenerateTable.append(tmpWallGenerateColumn)

            tmpNodeMatchingColumn = [True, os.path.join(DCMDirPath, 'solid/WallADINA/Wall.in'),
                                     os.path.join(DCMDirPath, 'solid/WallADINA/Wall.idb'),
                                     os.path.join(DCMDirPath, 'solid/WallFindFace/CompIfo.csv'),
                                     2, 10, 51, 90, 99, 9, 1, AUIExe, os.path.join(DCMDirPath, 'CFD/ADINA/CFD.idb'),
                                     os.path.join(DCMDirPath, 'CFD/result/CFD.por'),
                                     os.path.join(DCMDirPath, 'solid/WallNodeMatching')]
            self.NodeMatchingTable.append(tmpNodeMatchingColumn)

            tmpWallDataOutputColumn = [os.path.join(DCMDirPath, 'solid/WallDataOutput'), True, False,
                                       os.path.join(DCMDirPath, 'solid/WallADINA/Wall.idb'),
                                       os.path.join(DCMDirPath, 'solid/WallResult/Wall.por'), AUIExe, 4.5, 5.4]
            self.WallDataOutputTable.append(tmpWallDataOutputColumn)

            tmpWallVTUColumn = [os.path.join(DCMDirPath, 'solid/WallVtu'), True,
                                os.path.join(DCMDirPath, 'solid/WallFindFace/3Dmesh.nas'),
                                os.path.join(DCMDirPath, 'solid/WallDataOutput/Solid_lst_ParamsDic.npy'),
                                os.path.join(DCMDirPath, 'solid/WallVtu'), 'solid', False, True, True, True, True,
                                True, False, True, False, False]
            self.WallVTUTable.append(tmpWallVTUColumn)

        WallMask_df = pd.DataFrame(self.WallMaskTable, columns=WallMask_Head)
        WallMask_df.to_csv(self.WallMaskTablePath, index=None)

        WallStackTissue_df = pd.DataFrame(self.WallStackTissueTable, columns=WallStackTissue_Head)
        WallStackTissue_df.to_csv(self.WallStackTissueTablePath, index=None)

        WallSTL_df = pd.DataFrame(self.WallSTLTable, columns=WallSTL_Head)
        WallSTL_df.to_csv(self.WallSTLTablePath, index=None)

        LumenCut_df = pd.DataFrame(self.LumenCutTable, columns=LumenCut_Head)
        LumenCut_df.to_csv(self.LumenCutTablePath, index=None)

        WallSmooth_df = pd.DataFrame(self.WallSmoothTable, columns=WallSmooth_Head)
        WallSmooth_df.to_csv(self.WallSmoothTablePath, index=None)

        WallStackLumen_df = pd.DataFrame(self.WallStackLumenTable, columns=WallStackLumen_Head)
        WallStackLumen_df.to_csv(self.WallStackLumenTablePath, index=None)

        WallCut_df = pd.DataFrame(self.WallCutTable, columns=WallCut_Head)
        WallCut_df.to_csv(self.WallCutTablePath, index=None)

        WallFillGap_df = pd.DataFrame(self.WallFillGapTable, columns=WallFillGap_Head)
        WallFillGap_df.to_csv(self.WallFillGapTablePath, index=None)

        WallMesh_df = pd.DataFrame(self.WallMeshTable, columns=WallMesh_Head)
        WallMesh_df.to_csv(self.WallMeshTablePath, index=None)

        WallFindFace_df = pd.DataFrame(self.WallFindFaceTable, columns=WallFindFace_Head)
        WallFindFace_df.to_csv(self.WallFindFaceTablePath, index=None)

        WallCoo_df = pd.DataFrame(self.WallCooTable, columns=WallCoo_Head)
        WallCoo_df.to_csv(self.WallCooTablePath, index=None)

        WallElemID_df = pd.DataFrame(self.WallElemIDTable, columns=WallElemID_Head)
        WallElemID_df.to_csv(self.WallElemIDTablePath, index=None)

        WallGenerate_df = pd.DataFrame(self.WallGenerateTable, columns=WallGenerate_Head)
        WallGenerate_df.to_csv(self.WallGenerateTablePath, index=None)

        NodeMatching_df = pd.DataFrame(self.NodeMatchingTable, columns=NodeMatching_Head)
        NodeMatching_df.to_csv(self.NodeMatchingTablePath, index=None)

        WallDataOutput_df = pd.DataFrame(self.WallDataOutputTable, columns=WallDataOutput_Head)
        WallDataOutput_df.to_csv(self.WallDataOutputTablePath, index=None)

        WallVTU_df = pd.DataFrame(self.WallVTUTable, columns=WallVTU_Head)
        WallVTU_df.to_csv(self.WallVTUTablePath, index=None)
    ...
    def BatchMurraysLaw(self):
        ...
        # load DFs
        dfBloodPressureAndHeartRateTable = Pd_Funs.OpenDF(inPath=BloodPressureAndHeartRatePath, header=0,
                                                          usecols=None)
    ...
    def mainFunction(self):
        ...
        if self.ui.GenerateSolidTable_CB_CPP.isChecked():
            self.GenerateSolidTable()
            self.UpdateMsgLog("Generate Solid Table done")

        if self.ui.WallMask_CB_CPP.isChecked():
            cmd1 = self.CopyCommand(self.ui.WallMaskPath_Txt_BPP.toPlainText(), self.TableDir)
            os.system(cmd1)
            self.model.lumenCorrect.BatchCorrect(CSVPath=self.WallMaskTablePath)
            self.UpdateMsgLog("WallMask done")

        if self.ui.WallStackTissue_CB_CPP.isChecked():
            self.model.stackSegment.BatchProcessing(CSVPath=self.WallStackTissueTablePath)
            self.UpdateMsgLog("Wall Stack Tissue done")

        if self.ui.WallSTL_CB_CPP.isChecked():
            self.model.maskSTL.BatchConvert(CSVPath=self.WallSTLTablePath, choice=3)
            self.UpdateMsgLog("Wall STL done")

        if self.ui.LumenCut_CB_CPP.isChecked():
            self.model.Meshgeneration.batchrun(CSVPath=self.LumenCutTablePath)
            self.UpdateMsgLog("Lumen Cut done")

        if self.ui.WallSmooth_CB_CPP.isChecked():
            self.model.Meshgeneration.batchrun(CSVPath=self.WallSmoothTablePath)
            self.UpdateMsgLog("Wall Smooth done")

        if self.ui.WallStackLumen_CB_CPP.isChecked():
            self.model.StackSTL.batchrun(CSVPath=self.WallStackLumenTablePath)
            self.UpdateMsgLog("Wall Stack Lumen done")

        if self.ui.WallCut_CB_CPP.isChecked():
            self.model.MeshCut.batchrun(CSVPath=self.WallCutTablePath)
            self.UpdateMsgLog("Wall Cut done")

        if self.ui.WallFillGap_CB_CPP.isChecked():
            self.model.MeshFillGap.batchrun(CSVPath=self.WallFillGapTablePath)
            self.UpdateMsgLog("Wall Fill Gap done")

        if self.ui.WallMesh_CB_CPP.isChecked():
            self.model.Meshgeneration.batchrun(CSVPath=self.WallMeshTablePath)
            self.UpdateMsgLog("Wall Mesh done")

        if self.ui.WallFindFace_CB_CPP.isChecked():
            cmd1 = self.CopyCommand(self.ui.SolidsetPath_Txt_BPP.toPlainText(), self.TableDir)
            os.system(cmd1)
            self.model.SplitFaceElem.batchrun(CSVPath=self.WallFindFaceTablePath)
            self.UpdateMsgLog("Wall Find Face done")

        if self.ui.WallCoo_CB_CPP.isChecked():
            self.model.extractPointCoords.BatchExtract(CSVPath=self.WallCooTablePath)
            self.UpdateMsgLog("WallCoo done")

        if self.ui.WallElemID_CB_CPP.isChecked():
            self.model.TissueElemAssign.batchrun(CSVPath=self.WallElemIDTablePath)
            self.UpdateMsgLog("Wall Elem ID done")

        if self.ui.WallGenerate_CB_CPP.isChecked():
            self.model.Finfilegneration.batchsolidrun(CSVPath=self.WallGenerateTablePath)
            self.UpdateMsgLog("Wall Generate done")

        if self.ui.NodeMatching_CB_CPP.isChecked():
            self.model.NodeMatching.batchrun(CSVPath=self.NodeMatchingTablePath)
            self.UpdateMsgLog("Node Matching done")

        if self.ui.WallDataOutput_CB_CPP.isChecked():
            self.model.adina_DataOutput.batchrun(CSVPath=self.WallDataOutputTablePath)
            self.UpdateMsgLog("Wall Data Output done")

        if self.ui.WallVTU_CB_CPP.isChecked():
            self.model.SaveVTU.batchrun(CSVPath=self.WallVTUTablePath)
            self.UpdateMsgLog("Wall VTU done")
