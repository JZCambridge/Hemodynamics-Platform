...
class PlaqueQuantitativAnalysis:
    ...
    """
    def __init__(self,
                 spacing_x = 1.10, spacing_y = 1.10 , spacing_z = 1.10, InputFilePath = "", OutputFilePath = ""):
        #workpath
...
################################Normal##################################
    ...
    def NonBlanklayerIdGet(self):
        ...
        if list(set(self.BranchNumsListremove0)) == [2]:
            self.WebrtrnInfo["warningMessage"] += "\nThe current Mask has no cervical artery!"
    ...
    def BranchGrouping(self):
        ...
        for cont in range(len(self.BranchSliceGroup_ResetLabelDict)):
            ...
            if cont < self.bifurcation :
                ...
                # plt.imshow(Mask2)
                BranchGroupMask['branch' + str(2)].append(
                    Mask1)  # BranchGroupMask is a logical matrix here
                BranchGroupMask['branch' + str(3)].append(Mask2)
                ...
                if numpy.sum(MatrixSplited1):
                    self.BranchGroup['branch' + str(2)].append(MatrixSplited1)
                if numpy.sum(MatrixSplited2):
                    self.BranchGroup['branch' + str(3)].append(MatrixSplited2)
        ...
        for BGcont in range(len(self.BranchGroup)):
            try:
                ...
                self.BranchGroupStenosisDegree['branch' + str(BGcont +
                                                              1)] = \
                    1 - numpy.square(minimumLumenEqDiameter / tmpsliceLumenEqDiameterlistOri[
                        referenceWallEquivalentDiameterIndex])
    ...
    def wallProperty(self):
        ...
        try:
            ...
            self.WebrtrnInfo["errorMessage"] += "The current mask is reversed order!"
            self.JsonFileOutput()
            # sys.exit(0)
...
spacing_x = float(sys.argv[1])
spacing_y = float(sys.argv[2])
spacing_z = float(sys.argv[3])
work_dir = sys.argv[4]
...
PQA = PlaqueQuantitativAnalysis(spacing_x, spacing_y, spacing_z, work_dir, output_dir)
...
PQA.PlaqueAnalysisInfo["ReconstructionIndex"] = PQA.ReconstructionIndex
